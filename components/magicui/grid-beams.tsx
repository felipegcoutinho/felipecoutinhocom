...
interface GridBeamsProps extends HTMLAttributes<HTMLDivElement> {
  ...
  rayColor?: string; // space-separated RGB, e.g., "249 115 22"
  radialTint?: string; // CSS color for subtle radial tint
}
        ...
        style={
          {
            ...
            "--ray-color": rayColor,
            background:
              "linear-gradient(to bottom, rgb(var(--ray-color) / var(--ray-opacity)), rgb(var(--ray-color) / 0))",
      ...
      style={
        {
          ...
          "--radial-tint": radialTint,
          backgroundImage:
            "radial-gradient(ellipse at 50% -20%, var(--radial-tint), transparent 70%)",
        } as React.CSSProperties
      }
      {...props}
    >
